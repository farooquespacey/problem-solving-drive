...
public class _003_CanSum {
	...
	// Trying Pick and Don't pick concept (not efficient tho)
	// But good if we require result by applying all possible pair of numbers
	// i.e. for input 7 and {2,3}, The O/P is 2,5,4,7 with 2->3 applied, 2 applied,
	// 3 applied and none applied respectively
	static boolean canSumWithNoNumReusePickNotPick(int target, int[] numbers) {
		if (target == 0)
			return true;
		if (target < 0)
			return false;
		if (numbers.length == 0)
			return false;
		if (canSumWithNoNumReusePickNotPick(target - numbers[0], Arrays.copyOfRange(numbers, 1, numbers.length))
				|| canSumWithNoNumReusePickNotPick(target, Arrays.copyOfRange(numbers, 1, numbers.length)))
			return true;
		return false;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(canSumWithNoNumReusePickNotPick(6, new int[] { 3, 4, 5 }));
